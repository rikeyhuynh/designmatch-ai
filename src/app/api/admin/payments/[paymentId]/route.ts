...
export async function PATCH(request: Request, context: RouteContext) {
  ...
  if (authState.profile.role !== "admin") {
    return NextResponse.json(
      {
        message: "Bạn không có quyền xử lý payment.",
      },
      { status: 403 },
    );
  }
  ...
  if (action !== "confirm" && action !== "reject") {
    return NextResponse.json(
      {
        message: "Action không hợp lệ. Chỉ hỗ trợ confirm hoặc reject.",
      },
      { status: 400 },
    );
  }
  ...
  const { data: paymentData, error: paymentError } = await adminSupabase
    .from("payments")
    .select("id, job_id, amount_vnd, status")
    .eq("id", paymentId)
    .maybeSingle();
...
async function confirmPayment({
  ...
  const { error: updatePaymentError } = await adminSupabase
    ...
    .update({
      ...
      admin_note: note || "Admin confirmed manual bank transfer.",
    })
    .eq("id", payment.id);
...
async function rejectPayment({
  ...
  if (["confirmed", "paid", "completed"].includes(payment.status)) {
    return NextResponse.json(
      {
        message:
          "Payment đã được xác nhận nên không thể từ chối. Nếu cần hoàn tiền, hãy xử lý bằng quy trình refund riêng.",
      },
      { status: 400 },
    );
  }
  ...
  if (payment.status === "rejected") {
    return NextResponse.json({
      message: "Payment này đã bị từ chối trước đó.",
      paymentId: payment.id,
      jobId: job.id,
    });
  }
  ...
  const { error: updatePaymentError } = await adminSupabase
    ...
    .update({
      ...
      reviewed_at: now,
      reviewed_by: adminProfileId,
      admin_note: note || "Admin rejected this payment.",
    })
    .eq("id", payment.id);
  ...
  return NextResponse.json({
    message: "Đã từ chối payment. Job vẫn đang ở trạng thái chờ thanh toán.",
    paymentId: payment.id,
    jobId: job.id,
  });
}